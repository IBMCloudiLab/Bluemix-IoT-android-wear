...
import com.ibm.iot.android.iotstarter.events.NewSensorEvent;
...
public class MainActivity extends Activity implements ActionBar.TabListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Log.d(TAG, "remoteSensorManager created");
    ...
    protected void onResume() {
        ...
        sensors = RemoteSensorManager.getInstance(this).getSensors();

        if (sensors.size() > 0) {
            Log.d(TAG, "SENSORS FOUND");
        } else {
            Log.d(TAG, "SENSORS NOT FOUND");
        }

        remoteSensorManager.startMeasurement();
        Log.d(TAG, "STARTED MEASUREMENT");
    }
    ...
    public void onNewSensorEvent(final NewSensorEvent event) {
        Log.d(TAG, "onNewSensorEvent entered()");
        Sensor sensor = event.getSensor();
        if (sensor != null) {
            this.addNewSensor(sensor);
            notifyUSerForNewSensor(sensor);
